...
You are a **C# / .NET 10** developer working on **Zed.Web** — a companion NuGet library to `Zed` that provides ASP.NET Core web helpers. The library is published as the `Zed.Web` package.

**Repository:** `ztepsic/zed.web` on GitHub

## Tech Stack

- **C# 14, .NET 10**, nullable reference types enabled
- **ASP.NET Core** (`Microsoft.AspNetCore.App` FrameworkReference)
- **Newtonsoft.Json** for JSON serialization
- **Zed** (core library) — provides `AppError`, `ValidationError`, `HttpStatusCodeAppError`, FluentResults integration
- **FluentResults** (transitive via Zed) for error handling
...
- **JunitXml.TestLogger** for CI test reporting

### NuGet Feeds

- **nuget.org** — primary package source
- **GitHub Packages** (`nuget.pkg.github.com/ztepsic`) — pre-release (`*-alpha.*`) packages
...
- `Zed.Web.Tests` — unit tests for all library code (Extensions, Helpers, Models, Routes, Transaction)
...
- Name tests clearly: `MethodName_Scenario_ExpectedResult` (e.g., `ToProblemDetails_ThrowsInvalidOperationException_For_Successful_Result`).
...
Zed.Web is a companion library to `Zed` that provides ASP.NET Core web integration helpers:

```
Zed.Web/
  Extensions/    → FluentResultExtensions (Result → ProblemDetails conversion)
```

**Planned areas** (migrated from legacy `_old/Zed.Web`):

```
  Helpers/       → HtmlHelper, UrlHelper, Validation extensions
  Models/        → BreadcrumbsModel, PageInfoModel
  Routes/        → RouteHelper
  Transaction/   → UnitOfWorkAttribute, UnitOfWorkFilter (action filters)
```

### Key Dependencies

- **Zed** (`Zed.Errors.AppError`, `Zed.Errors.HttpStatusCodeAppError`, `Zed.Errors.ValidationError`) — error types used in FluentResult-to-ProblemDetails mapping
- **FluentResults** (`Result`, `Result<T>`) — the Result pattern that Zed.Web converts to HTTP responses
- **ASP.NET Core** (`ProblemDetails`, `ProblemDetailsFactory`, `ObjectResult`) — the target HTTP response types

### Legacy Code Reference

The `_old/` folder contains the original .NET Framework (ASP.NET MVC 5) implementation. Use it as a reference when migrating functionality to ASP.NET Core, but do **not** copy code verbatim — adapt patterns to ASP.NET Core idioms.

```
Zed.Web.Tests    → All unit tests for Zed.Web
```

This is a **library with zero application-layer dependencies** — it provides web integration helpers for downstream ASP.NET Core applications that use the `Zed` library.
